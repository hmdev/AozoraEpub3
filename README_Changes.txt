...
[1.0.7]
1.0.7u1
　- 表紙サイズが3:4でない場合に表示範囲がすれないようにダイアログ幅を可変に
　- 縮小時にjpeg圧縮率の指定が効いていなかった
　- 縮小時のpng保存修正
　　　出力をObjectPlanet PngEncoder(http://www.objectplanet.com/pngencoder/)に変更
　　　圧縮レベルを9(最高)に
　　　8bitグレースケールが薄くなるので8bitインデックスColorに
　　　2値縮小時の4bitインデックスの値修正
　- UbuntuとMacのDnD操作に対応
　- ログ削除時にクリップボードにコピー
　- 画像パスのIO例外は画像なし扱いで無視
　- 画面調整 (上下をSplitでサイズ変更可能に テキスト幅をフォントサイズで調整)
　- kindlegen.exeがjarと同じパスにあれば変換後実行
　　※画像のみは全画面設定向けのスタイルにしているため変換前のepubは利用不可
