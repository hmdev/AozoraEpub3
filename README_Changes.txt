...
------------
[1.1.0]
1.0.7→1.1.0bでの主な変更点 (2015/01/13)
...
　　　gaiji/ に1文字フォントファイル 'u'+Unicode+'.ttf' があり本文で使われている文字のみ組み込む
　　　　文字を〓で出力してフォント指定用のspanタグで括る ( <span class="glyph u2496d">〓</span> )
　　　IVS付きの文字の1文字フォント置換にも対応(U+FE00-FE0F と U+E0100-E01EF)
　　　※フォントによっては細かったり上下に空白ができる場合有り → Fontforgeで調整が必要？
　- IVSの出力設定
　　　(U+FE00-FE0E と U+E0100-E01EF)を選択
　　　IVSの出力または除外をログに出力
　　　IVS出力時は正立指定 (-epub-text-orientation:upright;)
　- Readerで正立しない文字を縦中横していた部分を正立に変更
　　※Readerのファームアップで直ったがDigitalEditions向けに残す (÷±∞∴∵ は正立指定のまま)
　- 表紙画像が無い場合に入力ファイルパスに cover.png|jpg|jpeg のファイルがあれば確認画面で表紙に設定
　- ルビの終了と開始が連続する場合は </ruby><ruby> タグは追加しない
　- 拡張ラテン文字Aの定義を追加＋CID無しでエラー出力しない
　- 特殊文字のエスケープ処理を調整（特殊文字の前後の自動縦中横されないバグを修正）
　- JIS変換調整後に一部半角になってしまっていたのを修正
