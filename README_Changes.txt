...
　- 外字の画像出力
　- IVS対応？ (青空注記はchuki_utf.txtの定義で対応可？ 対応しない端末の調査と文字除去)
...
　- Rarファイル読み込み
　- Web小説のURL末尾の"/"チェックを修正
　- Web小説のキャッシュ内でフォルダが作れない場合は同名ファイルを削除
　- ひらがな＋濁点/半濁点を縦中横表示(半角文字を利用)
　- ルビの文字種判別で漢字の間の「ノカケヵヶ」は漢字扱い
　- 前方参照型の後ろにルビがあったら前に移動
　- 罫囲みがはみ出ないようにスタイル調整(display:inline-block;を削除)
1.1.0b33 (2015/01/06)
