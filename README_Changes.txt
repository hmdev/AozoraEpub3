...
[1.0.7]
1.0.7u1
　- 縮小・余白除去時にjpeg圧縮率の指定が効いていなかった
　- UbuntuのDnD操作に対応
　- ログ削除時にクリップボードにコピー
　- 画像パスのIO例外は画像なし扱いで無視
　- 画面調整 (上下をSplitでサイズ変更可能に テキスト幅をフォントサイズで調整)
　- Windows用exeラッパーの作成とjar起動時の引数で変換 (「送る」で起動して変換)
　- kindlegen.exeがjarと同じパスにあれば変換後実行(オプション無し)
　- kindlegen.exeがある場合表紙はjpegで保存、画像のみなら全画面metaタグを追加
