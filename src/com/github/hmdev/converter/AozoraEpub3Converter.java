...
public class AozoraEpub3Converter
{
			...
			while ((line = src.readLine()) != null) {
				...
				if (line.length() > 0 && line.charAt(0)!='#') {
					try {
						...
						LogAppender.append("[ERROR] "+chukiTagFile.getName()+" ("+(lineNum+1)+") : "+line+"\n");
					}
				}
			}
		} finally {
					...
					if (line.length() > 0 && line.charAt(0)!='#') {
						try {
							...
							if (values[0].length() == 1) {
								...
								LogAppender.append("[ERROR] "+replaceFile.getName()+" ("+(lineNum+1)+") too long : "+line+"\n");
							}
						} catch (Exception e) {
							LogAppender.append("[ERROR] "+replaceFile.getName()+" ("+(lineNum+1)+") : "+line+"\n");
						}
					}
				}
			} finally {
	...
	public BookInfo getBookInfo(BufferedReader src, ImageInfoReader imageInfoReader, TitleType titleType) throws Exception
	{
		...
		while ((line = src.readLine()) != null) {
			...
			if (line.startsWith("--------------------------------")) {
				if (!line.startsWith("--------------------------------------------------")) {
					LogAppender.append("[WARN] コメント行の文字数が足りません ("+(lineNum+1)+")\n");
				} else {
					...
					if (inComment) {
						//コメント行終了
						if (commentLineNum > 20) LogAppender.append("[WARN] コメントが "+commentLineNum+" 行 ("+(commentLineStart+1)+"-"+(lineNum+1)+")\n");
						commentLineNum = 0;
			...
			if (this.chapterSection && addSectionChapter) {
				if (lineNum == bookInfo.titleLine || lineNum > bookInfo.titleEndLine) {
					...
					if (name.replaceAll("◇|◆|□|■|＊|＋", "").length() > 0) {
						bookInfo.addChapterLineInfo(new ChapterLineInfo(lineNum, 1, lastEmptyLine==lineNum-1, name, true));
						if (this.useNextLineChapterName) addNextChapterName = true;
						addSectionChapter = false; //改ページ後のChapter出力を抑止
					}
				}
			...
			while (m.find()) {
				...
				if (chukiFlagPageBreak.contains(chukiName)) {
					...
					if (line.length() == m.start()+chukiTag.length())  {
						preChapterLineInfo = new ChapterLineInfo(lineNum+1, chapterChukiMap.get(chukiName), lastEmptyLine==lineNum-1);
						bookInfo.addChapterLineInfo(preChapterLineInfo);
						addChapterName = true; //次の行を見出しとして利用
						addNextChapterName = false;
					}
					else {
						bookInfo.addChapterLineInfo(new ChapterLineInfo(lineNum, chapterChukiMap.get(chukiName), lastEmptyLine==lineNum-1, this.getChapterName(line.substring(m.end()))) );
			...
			if (autoChapter && bookInfo.getChapterLevel(lineNum) == 0) {
				...
				if (this.chapterPattern != null) {
					if (this.chapterPattern.matcher(noChukiLine).find()) {
						bookInfo.addChapterLineInfo(new ChapterLineInfo(lineNum, 13, lastEmptyLine==lineNum-1, this.getChapterName(line)));
						if (this.useNextLineChapterName) addNextChapterName = true; //次の行を連結
						addSectionChapter = false; //改ページ後のChapter出力を抑止
					}
				}
	...
	public void convertTextToEpub3(BufferedWriter out, BufferedReader src, BookInfo bookInfo) throws Exception
	{
		...
		this.bookInfo = null;
			...
			e.printStackTrace();
			LogAppender.append("[ERROR] ("+this.lineNum+")\n");
			throw e;
		}
	}
		...
		do {
			...
			if (chuki.charAt(0) == '※') {
				...
				if (gaiji != null && gaiji.getBytes().length ==4) {
					if (logged) LogAppender.append("外字4バイト: ("+(lineNum+1)+") "+chuki+"\n");
					if (!gaiji32) {
						gaiji = "〓";
						if (!isInnerChuki(line, m.start())) {
				...
				if (gaiji == null) {
					if (isInnerChuki(line, m.start())) {
						...
						if (imageStartIdx > -1 && chuki.indexOf('.', 2) != -1) {
							...
							if (logged) LogAppender.append("外字画像利用: ("+(lineNum+1)+") "+chuki+"\n");
						} else {
							//画像以外
							if (logged) LogAppender.append("外字未変換: ("+(lineNum+1)+") "+chuki+"\n");
							gaiji = "〓［＃行右小書き］（"+chukiValues[0]+"）［＃行右小書き終わり］";
						}
					}
				}
		...
		while (m.find()) {
			...
			if (tags != null) {
				...
				if (chukiFlagPageBreak.contains(chukiName) && !bookInfo.isNoPageBreakLine(lineNum)) {
					//字下げ状態エラー出力
					if (inJisage >= 0) {
						LogAppender.append("字下げ注記省略: ("+inJisage+") \n");
						//字下げ省略処理
						//字下げフラグ処理
				...
				//字下げフラグ処理
					...
					if (tags.length > 1) inJisage = -1;//インライン
					 ...
					 if (inJisage == -1) {
						 LogAppender.append("字下げ注記省略：("+(lineNum+1)+")\n");
						 noAppend = true;
					 }
					inJisage = -1;
				}
				...
				if (imageStartIdx > -1) {
					...
					if (imageStartIdx == 2 && chukiTag.endsWith("）］") && chukiTag.indexOf('.', 2) == -1) {
						...
						//拡張子を含まない
						LogAppender.append("注記未変換: ("+(lineNum+1)+") "+chukiTag+"\n");
					} else {
						//画像ファイル名置換処理実行
						String srcFilePath = this.getImageChukiFileName(chukiTag, imageStartIdx);
						if (srcFilePath == null) {
							LogAppender.append("注記エラー: ("+(lineNum+1)+") "+chukiTag+"\n");
						} else {
					...
					if (srcFilePath == null) {
						LogAppender.append("画像注記エラー: ("+(lineNum+1)+") "+chukiTag+"\n");
					} else {
						//単ページ画像の場合は<p>タグを出さない
					...
					if (!patternMatched) {
						...
						if (m2.find()) {
							if (inJisage == -1) LogAppender.append("字下げ注記エラー：("+(lineNum+1)+")\n");
